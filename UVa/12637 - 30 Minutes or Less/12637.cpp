...
class Solution {
  public:
    ...
    int prev_ious[MAX_SIZE]; //array for memorizing alternating p
    ...
    int assignmentProblem(vector<double> Arr, int N) {
        n = N;
...
int main() {
    vector<Point> restaurantPositions;
    vector<Point> pizzaOrdersPositions;

    int restaurantAmount;
    int pizzaOrdersAmount;
    int n;

    while (cin >> restaurantAmount >> pizzaOrdersAmount) {
        if (restaurantAmount >= pizzaOrdersAmount) {
            n = restaurantAmount;
        } else {
            n = pizzaOrdersAmount;
        }

        restaurantPositions.resize(n+1);
        pizzaOrdersPositions.resize(n+1);
        ...
        for (int i = 0; i < restaurantAmount; i++) {
            ...
            restaurantPositions[i] = Point(x, y);
        }

        for (int i = 0; i < pizzaOrdersAmount; i++) {
            int x, y;
            cin >> x >> y;

            pizzaOrdersPositions[i] = Point(x, y);
        }   

        for (int i = 0; i < restaurantAmount; i++) {
            for (int j = 0; j < pizzaOrdersAmount; j++) {
                Arr[i*n+j] = manhattanDistance(restaurantPositions[i], pizzaOrdersPositions[j]);
            }   
        }
